...
- Add it to or edit it in src/environments/environment.dev.ts to expose it to your app as e.g. environment.API_URL:
  ```typescript
...
The ngx-rocket `env` task solves this problem really well, and avoids the need for separate `environment.dev.ts` files for
deployment-specific configuration. 

To add a deployment-specific configuration:

1. edit the existing `environment.ts.ts` files for whichever environments you'd like to make that variable
